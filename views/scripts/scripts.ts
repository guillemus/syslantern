...
function drawHistory(canvas: HTMLCanvasElement, values: number[], stroke: string, fill: string) {
  ...
  const yFor = (value: number) => rect.height - (Math.max(0, Math.min(100, value)) / 100) * rect.height
...
class OpenLogsCPUHistory extends HTMLElement {
  ...
  private render() {
    ...
    const load = current ? `load ${current.load1M.toFixed(2)} . ${current.coresLogical} logical cores` : "Waiting for CPU history"
  ...
  private renderCoreRows(points: CPUPoint[]) {
    ...
    for (let core = 0; core < coreCount; core++) {
      const values = points.map((point) => point.perCorePercent[core] ?? point.usedPercent).slice(-72)
...
class OpenLogsDiskPressure extends HTMLElement {
  ...
  private render() {
    ...
    const worst = disks[0]
    const badge = worst ? `${worst.mount || "disk"} ${worst.usedPercent >= 85 ? "needs attention" : "healthy"}` : "waiting"
  ...
  private diskRow(disk: DiskPoint) {
    const tone = disk.usedPercent >= 85 ? "text-red-400" : disk.usedPercent >= 65 ? "text-amber-400" : "text-emerald-400"
    const fill = disk.usedPercent >= 85 ? "bg-red-400" : disk.usedPercent >= 65 ? "bg-amber-400" : "bg-emerald-400"
